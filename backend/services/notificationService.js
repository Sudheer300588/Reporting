...
class NotificationService {
  ...
  async createNotification(data) {
    try {
      ...
      if (!prisma.notification) {
        logger.info(
          "Notification table not found, skipping notification creation",
        );
        return null;
      }
  ...
  async sendEmail(notification) {
    try {
      ...
      if (!recipient || !recipient.email) {
        logger.warn("No email found for recipient", {
  ...
  async scheduleDeadlineReminders() {
    try {
      ...
      logger.info("Scheduled deadline reminders", {
        urgent: urgentTasks.length,
        upcoming: upcomingTasks.length,
      });
    } catch (error) {
      logger.error("Error scheduling deadline reminders", {
        error: error.message,
      });
    }
  }
