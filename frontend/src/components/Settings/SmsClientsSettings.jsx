...
import { MessageSquare, Eye, EyeOff, RefreshCw, Pencil, Trash2, ExternalLink, Plus, CheckCircle, XCircle } from 'lucide-react';
...
const SmsClientsSettings = () => {
  ...
  const handleDelete = async (clientId) => {
    if (!confirm('⚠️ Are you sure you want to delete this SMS client? All associated SMS campaigns will be removed.')) {
      return;
    }

    const toastId = toast.loading('⚙️ Deleting SMS client...');
    try {
      const baseUrl = import.meta.env.VITE_API_URL || "";
      await axios.delete(`${baseUrl}/api/mautic/sms-clients/${clientId}`);
      toast.update(toastId, { render: '✅ SMS client deleted successfully!', type: 'success', isLoading: false, autoClose: 3000 });
      await fetchClients();
    } catch (error) {
